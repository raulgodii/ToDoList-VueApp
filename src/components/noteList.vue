  ...
  <ul>
    <li v-for="(element, index) in filterElements" :key="element.date" :class="{ done: element.done }">
      ...
      <div @click="changePriority(element, element.id)" style="padding: 0px 20px; cursor: pointer;">
        ...
        <p><span class="priority" :class="checkPriority(element.priority)">{{ checkPriority(element.priority) }}</span><span
            style="color: rgb(116, 116, 116); font-size: 0.8em;" class="date">{{ element.date }} </span>
            
        </p>
        <p style="display: block; text-decoration: none;" id="uid" v-if="esAdmin">UID: {{ element.uid }}</p>
      </div>
      <button class="btnD" @click="deleteElement(element, element.id)">
